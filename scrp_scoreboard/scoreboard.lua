...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if IsControlPressed(0, Config.key) and IsInputDisabled(0) then
            if not open then
                MyKey = GetGameTimer()
                TriggerServerEvent("scrp_scoreboard:GetBoard", GetPlayers(), MyKey)
                open = true
                while open do
                    Wait(0)
...
RegisterNetEvent("scrp_scoreboard:Show")
AddEventHandler("scrp_scoreboard:Show", function(html, key)
    if MyKey == key then
        DisplayHud(false)
        DisplayRadar(false)
        local players = {}
