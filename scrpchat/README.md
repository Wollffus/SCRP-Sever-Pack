...
![scrpchat screenshot](screenshot.png)
...
If you add/remove the ace for players while the server is running, you must run `scrpchat_refresh_perms` or restart the resource to apply the changes.
...
3. Grant the scrpchat resource any aces needed to execute the desired commands.
...
add_ace resource.scrpchat command.kick allow
add_ace resource.scrpchat command.ban allow
```
