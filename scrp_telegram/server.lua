...
RegisterServerEvent("scrp_Telegram:GetMessages")
AddEventHandler("scrp_Telegram:GetMessages", function(src)
	local _source
	...
	if redemrp then 
		TriggerEvent('redemrp:getPlayerFromId', _source, function(user)
			...
			MySQL.Async.fetchAll("SELECT * FROM telegrams WHERE recipient=@recipient AND recipientid=@recipientid ORDER BY id DESC", { ['@recipient'] = recipient, ['@recipientid'] = recipientid }, function(result)
				TriggerClientEvent("scrp_Telegram:ReturnMessages", _source, result)
			end)
		end)
	else 
...
RegisterServerEvent("scrp_Telegram:SendMessage")
AddEventHandler("scrp_Telegram:SendMessage", function(firstname, lastname, message, players)
...
RegisterServerEvent("scrp_Telegram:DeleteMessage")
AddEventHandler("scrp_Telegram:DeleteMessage", function(id)
	local _source = source

	if redemrp then 
		MySQL.Async.execute("DELETE FROM telegrams WHERE id=@id",  { ['@id'] = id }, function(count)
			if count > 0 then 
				TriggerEvent("scrp_Telegram:GetMessages", _source)
			else
