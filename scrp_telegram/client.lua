...
local locations = {
    { x=-174.2, y=633.2, z=114.09 }, -- Valentine train station
    { x=1225.57, y=-1293.87, z=76.91 }, -- Rhodes train station
    { x=2731.55, y=-1402.37, z=46.18 }, -- Saint Denis train station
    { x=-5519.76, y=-2909.16, z=-1.75 }, -- Tumbleweed Saloon
    { x=-875.07, y=-1328.75, z=43.96 }, -- Blackwater Station
    { x=-1817.43, y=-370.79, z=163.3 }, -- Strawberry Welcome Center
}

RegisterNetEvent("scrp_Telegram:ReturnMessages")
AddEventHandler("scrp_Telegram:ReturnMessages", function(data)
    index = 1
    telegrams = data
           ...
           if IsPlayerNearCoords(value.x, value.y, value.z) then
                if not menu then
                    ...
                    if IsControlJustReleased(0, 0x760A9C6F) then
                        menu = true
                        TriggerServerEvent("scrp_Telegram:GetMessages")
                    end
                end
            end
        end
...
RegisterNUICallback('delete', function()
    TriggerServerEvent("scrp_Telegram:DeleteMessage", telegrams[index].id)
end)
...
function GetMessage(firstname, lastname)
    ...
    while (UpdateOnscreenKeyboard() == 1) do
        ...
        if (GetOnscreenKeyboardResult()) then
            ...
            TriggerServerEvent("scrp_Telegram:SendMessage", firstname, lastname, message, GetPlayerServerIds())
