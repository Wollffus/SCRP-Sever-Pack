...
Citizen.CreateThread(function()
    for _, zone in pairs(Config.Marker) do
        TriggerEvent("scrp_stagecoach:CreateNPC", zone)
...
RegisterNetEvent("scrp_stagecoach:CreateNPC")
AddEventHandler("scrp_stagecoach:CreateNPC", function (zone)
...
RegisterNetEvent("scrp_stagecoach:successful_dropoff")
AddEventHandler("scrp_stagecoach:successful_dropoff", function (fare, npc_id)

    while true do
    
        TriggerServerEvent("scrp_stagecoach:pay_fare", fare)
        local fare_paid = true
        RemoveBlip(p1)
        ClearGpsMultiRoute()
        passenger_spawned = false
        TriggerEvent("scrp_stagecoach:StartCoachJob", zone_name, spawn_coach, passenger_spawned)
        Wait(30000)
        DeleteEntity(npc_id)
...
RegisterNetEvent("scrp_stagecoach:unsuccessful_dropoff")
AddEventHandler("scrp_stagecoach:unsuccessful_dropoff", function (fare, npc_id)
...
RegisterNetEvent("scrp_stagecoach:PassengerOnboard")
AddEventHandler("scrp_stagecoach:PassengerOnboard", function (zone_name, route, spawn_coach, repair_active)
    ...
    Wait(10)   
        ...
        if GetDistanceBetweenCoords(Config.Destination[zone_name][route].x, Config.Destination[zone_name][route].y, Config.Destination[zone_name][route].z, GetEntityCoords(passenger),false)<5 and passenger_onboard ~= false then
            ...
            TriggerEvent("scrp_stagecoach:successful_dropoff", fare_amount, npc_id)
            passenger_onboard = false
            
        end       

        if IsEntityDead(passenger) then
            TriggerEvent("scrp_stagecoach:unsuccessful_dropoff", 0, npc_id)
            passenger_onboard = false
        end
        ...
        if GetVehicleBodyHealth(spawn_coach) == 0 or IsVehicleDriveable(spawn_coach) == false then
            ...
            TriggerEvent("scrp_stagecoach:replace_stagecoach", spawn_coach, repair_active)
            TaskLeaveVehicle(passenger, spawn_coach, 0)
            TriggerEvent("scrp_stagecoach:unsuccessful_dropoff", 0, npc_id, spawn_coach)
        end
...
RegisterNetEvent("scrp_stagecoach:AddDriverBlip")
AddEventHandler("scrp_stagecoach:AddDriverBlip", function (coach_driver, coach_driver_blip)
...
RegisterNetEvent("scrp_stagecoach:StartCoachJob")
AddEventHandler("scrp_stagecoach:StartCoachJob", function (zone_name, spawn_coach, driving)
    ...
    TriggerServerEvent('scrp_stagecoach:SendDriverEntity', coach_driver)
    ...
    Wait(10)
        ...
        if GetDistanceBetweenCoords(Config.PickUp[zone_name][route].x, Config.PickUp[zone_name][route].y, Config.PickUp[zone_name][route].z, GetEntityCoords(PlayerPedId()),false)<10 then
            ...
            passenger_onboard = true
            RemoveBlip(p1)
            TriggerEvent("scrp_stagecoach:PassengerOnboard", zone_name, route, spawn_coach)
...
RegisterNetEvent("scrp_stagecoach:SpawnWagon")
AddEventHandler("scrp_stagecoach:SpawnWagon", function (_model)
    ...
    EndStageCoachCam()
    driving = true
    TriggerServerEvent("scrp_stagecoach:StartCoachJobServer", zone_name, spawn_coach, driving)

end)

-- Stop Driving Menu Event

RegisterNetEvent("scrp_stagecoach:stop_driving")
AddEventHandler("scrp_stagecoach:stop_driving", function (spawn_coach)
...
RegisterNetEvent("scrp_stagecoach:replace_stagecoach")
AddEventHandler("scrp_stagecoach:replace_stagecoach", function (spawn_coach, repair_active)
    ...
    while true do
        ...
        if PromptHasHoldModeCompleted(RepairCoachPrompt) then
            ...
            driving = true
            TriggerEvent("scrp_stagecoach:StartCoachJob", zone_name, spawn_coach, driving)
        end 
    end
end)
        ...
        if WarMenu.IsMenuOpened('Stagecoach') then
            for i = 1, #Coaches do
                if WarMenu.Button(Coaches[i]['Text'], Coaches[i]['SubText'], Coaches[i]['Desc']) then
                    TriggerServerEvent('scrp_stagecoach:buy_stagecoach', Coaches[i]['Param'])
                    Citizen.Wait(500)
                    WarMenu.CloseMenu()
                end
            end
    ...
    while true do
        ...
        if WarMenu.IsMenuOpened('DrivingStatus') then
            ...
            if WarMenu.Button("Stop Driving") then
                    TriggerEvent("scrp_stagecoach:stop_driving")
                    WarMenu.CloseMenu()
                    Wait(600)
                    WarMenu.Display()
    ...
    while true do
        ...
        if WarMenu.IsMenuOpened('DrivingStatusFalse') then
            ...
            if WarMenu.Button("Start Driving") then
                    TriggerServerEvent("scrp_stagecoach:StartCoachJobServer", zone_name, spawn_coach, true)
                    WarMenu.CloseMenu()
                    Wait(600)
                    WarMenu.Display()
            elseif WarMenu.Button("Exit") then
...
Citizen.CreateThread(function(HasStagecoaches)
    ...
    while true do
        ...
        if WarMenu.IsMenuOpened('Stagecoach_MainMenu') then
            ...
            if WarMenu.MenuButton('Owned Coaches', 'ListStagecoaches') then TriggerServerEvent('scrp_stagecoach:loadstagecoach') end
...
RegisterNetEvent("scrp_stagecoach:LoadCoachesMenu")
AddEventHandler("scrp_stagecoach:LoadCoachesMenu", function (HasStagecoaches)
        ...
        if WarMenu.IsMenuOpened('ListStagecoaches') then
            for k, v in pairs(Stagecoaches) do 
                if WarMenu.Button(v['name']) then
                    TriggerEvent('scrp_stagecoach:SpawnWagon', v['stagecoach'])
                    WarMenu.CloseMenu()
                    Wait(600)
                    WarMenu.Display()
                end
    ...
    Citizen.CreateThread( function()
        while true do
            ...
            elseif invehicle == nil then
                Citizen.Wait(1000)
                TriggerServerEvent("scrp_stagecoach:pay_fare", fare_amount)
                break
            end
        end
    end)
