...
RegisterNetEvent('scrpchat:staffMessage')
RegisterNetEvent('scrpchat:globalMessage')
RegisterNetEvent('scrpchat:actionMessage')
RegisterNetEvent('scrpchat:whisperMessage')
RegisterNetEvent('scrpchat:getPermissions')
RegisterNetEvent('scrpchat:report')
RegisterNetEvent('scrpchat:sendToDiscord')
...
function Log(label, message)
	...
	print(string.format('%s[scrpchat] %s[%s]%s %s', LogColors.name, color, label, LogColors.default, message))
end
...
function LocalMessage(source, message)
	...
	TriggerClientEvent('scrpchat:localMessage', -1, source, name, color, message)
end
...
AddEventHandler('scrpchat:globalMessage', function(message)
...
AddEventHandler('scrpchat:actionMessage', function(message)
	local name = GetName(source)
	...
	TriggerClientEvent("scrpchat:action", -1, source, name, message)
end, false)
...
AddEventHandler('scrpchat:whisperMessage', function(id, message)
	local name, color = GetNameWithRoleAndColor(source)
	...
	if id then
		...
		TriggerClientEvent('scrpchat:whisperEcho', source, id, GetName(id), message)
		-- Send the message to the recipient
		TriggerClientEvent('scrpchat:whisper', id, source, name, message)
		-- Set the /reply target for sender and recipient
		TriggerClientEvent('scrpchat:setReplyTo', id, source)
		TriggerClientEvent('scrpchat:setReplyTo', source, id)
	else
		TriggerClientEvent('scrpchat:whisperError', source, id)
	end
end)
...
AddEventHandler('scrpchat:staffMessage', function(message)
	StaffMessage(source, message)
end)

function SetPermissions(source)
	TriggerClientEvent('scrpchat:setPermissions', source, {
		canAccessStaffChannel = IsPlayerAceAllowed(source, ServerConfig.StaffChannelAce)
	})
end

AddEventHandler('scrpchat:getPermissions', function()
	SetPermissions(source)
end)

RegisterCommand('scrpchat_refresh_perms', function(source, args, raw)
	for _, playerId in ipairs(GetPlayers()) do
...
AddEventHandler('scrpchat:report', function(player, reason)
...
AddEventHandler('scrpchat:sendToDiscord', SendToDiscord)
